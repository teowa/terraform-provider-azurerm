...
* `secure_boot_enabled` - (Optional) Whether `secure boot` is enabled for this Node Pool. `temporary_name_for_rotation` must be specified when changing this property.
...
* `vtpm_enabled` - (Optional) Whether `vTPM` is enabled for this Node Pool. `temporary_name_for_rotation` must be specified when changing this property.
