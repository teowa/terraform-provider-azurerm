...
* `preconfigured_nsg_enabled` - (Optional) Whether to use preconfigured Network Security Group. Possible values are `true` and `false`. Defaults to `false`. Changing this forces a new resource to be created.
