...
* `address_prefix` (Optional) Specifies the address prefix.

* `address_prefix_type` (Optional) Specifies the address prefix type. Possible values are `IPPrefix` and `ServiceTag`.
