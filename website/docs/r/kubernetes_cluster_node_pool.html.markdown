...
* `secure_boot_enabled` - (Optional) Whether `secure boot` is enabled for this Node Pool. Changing this property requires specifying `temporary_name_for_rotation`.
...
* `vtpm_enabled` - (Optional) Whether `vTPM` is enabled for this Node Pool. Changing this property requires specifying `temporary_name_for_rotation`.
