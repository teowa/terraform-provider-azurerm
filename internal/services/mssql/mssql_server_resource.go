...
func resourceMsSqlServerUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	// NOTE: this must happen before the `administrator_login_password` update below, since the API will reject a
	// password update while `azuread_authentication_only` is still enabled.
