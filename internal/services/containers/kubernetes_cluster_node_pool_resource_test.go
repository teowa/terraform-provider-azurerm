...
func TestAccKubernetesClusterNodePool_artifactStreamingEnabled(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster_node_pool", "test")
	r := KubernetesClusterNodePoolResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.artifactStreamingEnabled(data, true),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
		...
		data.ImportStep(),
		{
			Config: r.artifactStreamingEnabled(data, false),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
...
func (r KubernetesClusterNodePoolResource) artifactStreamingEnabled(data acceptance.TestData, enabled bool) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
