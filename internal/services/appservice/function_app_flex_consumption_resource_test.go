...
func TestAccFunctionAppFlexConsumption_siteUpdateStrategyUpdate(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_function_app_flex_consumption", "test")
	r := FunctionAppFlexConsumptionResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep("site_credential.0.password"),
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("site_update_strategy").HasValue("RollingUpdate"),
			),
		},
		data.ImportStep("site_credential.0.password"),
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("site_update_strategy").HasValue("Recreate"),
			),
		},
		data.ImportStep("site_credential.0.password"),
	})
...
func (r FunctionAppFlexConsumptionResource) siteUpdateStrategy(data acceptance.TestData, siteUpdateStrategy string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
`, r.template(data), data.RandomInteger, siteUpdateStrategy)
}
