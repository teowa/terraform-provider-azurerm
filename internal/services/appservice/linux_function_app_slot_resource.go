		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			siteEnvelope := webapps.Site{
				...
				Properties: &webapps.SiteProperties{
					...
					OutboundVnetRouting:  helpers.ExpandOutboundVnetRouting(siteConfig.VnetRouteAllEnabled, pointer.To(functionAppSlot.VnetImagePullEnabled), pointer.To(functionAppSlot.VirtualNetworkBackupRestoreEnabled)), // (@jackofallops) - Value appear to need to be set in both SiteProperties and SiteConfig for now? https://github.com/Azure/azure-rest-api-specs/issues/24681
				},
			}
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model := functionAppSlot.Model; model != nil {
				...
				if props := model.Properties; props != nil {
					...
					state.VirtualNetworkBackupRestoreEnabled = pointer.From(helpers.FlattenVnetBackupRestoreEnabled(props.OutboundVnetRouting))
					state.VnetImagePullEnabled = pointer.From(helpers.FlattenVnetImagePullEnabled(props.OutboundVnetRouting))
	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if metadata.ResourceData.HasChange("site_config") {
				...
				model.Properties.OutboundVnetRouting.ApplicationTraffic = model.Properties.SiteConfig.VnetRouteAllEnabled
			}
