...
func NewNestedItemID(configurationStoreBaseURL, key, label string) (*NestedItemId, error) {
	// configurationStoreBaseURL example: https://testappconf1.azconfig.io
	configurationURL, err := url.ParseRequestURI(configurationStoreBaseURL)
	if err != nil {
		return nil, fmt.Errorf("parsing %q: %+v", configurationURL, err)
	}

	return &NestedItemId{
		ConfigurationStoreBaseURL: configurationURL.String(),
		Key:                       key,
		Label:                     label,
	}, nil
}
...
func (id NestedItemId) ID() string {
	...
	baseURL, _ := url.ParseRequestURI(id.ConfigurationStoreBaseURL)
	u := &url.URL{
...
func (id NestedItemId) String() string {
	components := []string{
		fmt.Sprintf("Base URL %q", id.ConfigurationStoreBaseURL),
...
func ParseNestedItemID(input string) (*NestedItemId, error) {
	...
	childId := NestedItemId{
		ConfigurationStoreBaseURL: fmt.Sprintf("%s://%s", idURL.Scheme, idURL.Host),
		Key:                       key,
		Label:                     label,
	}
