...
func TestAccOpenShiftCluster_managedOutboundIPs(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_redhat_openshift_cluster", "test")
	r := OpenShiftClusterResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.managedOutboundIPs(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("network_profile.0.managed_outbound_ip_count").HasValue("2"),
			),
		},
		data.ImportStep("service_principal.0.client_secret"),
	})
...
func TestAccOpenShiftCluster_withUserAssignedIdentity(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_redhat_openshift_cluster", "test")
	r := OpenShiftClusterResource{}
...
func (r OpenShiftClusterResource) managedOutboundIPs(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
...
func (r OpenShiftClusterResource) withUserAssignedIdentity(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
